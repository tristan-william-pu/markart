...
question:{
  ...
  forbidden:'If you set the quick deal to \'disabled\' then you will need to confirm any trading behavior in a pop-up window.<br>You can also set default stop/stop and market price range settings for all order types in this window.',
  ...
  wap:'The depth of market refers to the liquidity that a currency pair can use at different price points. There are three market depths available in CPT cTrader:',
  standard:'The VWAP DoM shows an estimated VWAP price list after the adjustable volume.',
  price:' Standard DoM is a summary of the available liquidity for a particular tool. The amount of liquidity is shown after each available price.',
  notLogin:' Price DoM shows the list of prices above and below the current price and the available liquidity behind each price.',
  enter:'Why can\'t I log in to CPT cTrader?',
  according:'Please verify that you have entered the correct CPT cTrader ID (usually your email address) and password. If you are downloading CPT cTrader on the CPT Markets website, you can only log in to the CPT cTrader platform. Please note that you cannot log in to CPT cTrader using an MT4 or MT5 account and vice versa.',
  code:'How to trade according to the chart?',
  target:'You can modify Stop Loss, Take Profit and Limit Order on the chart in CPT cTrader. Open the chart of the symbol you want to trade and click the View Options icon at the top of the chart. Select "Orders" to view the entry price, volume, and direction on the chart. To modify a position, place the cursor on the line where Stop Loss, Take Profit or Trading Volume is on the chart, click and drag it to the desired ratio.',
  targetqa:'What is the market sentiment indicator?',
  recovery:'Market sentiment shows long and short orders for other traders. You can use this tool to determine the percentage of bearish and bullish traders before making a decision to enter the market.',
  stips:'How can I restore my CPT cTrader ID?',
  install:'You can reset your CPT cTrader ID password by following the steps below: First you need to open the CPT cTrader platform. Click on the Login button and you will enter the new login page and click on the "Forgot Password?" button in the password box. You will be taken to the password reset screen where you can enter your email address to create your CPT cTrader ID. Check your new password in your mailbox, don\'t forget to change your password and remember.',
  desk:'How to install CPT cTrader desktop version?',
  webLink:'You can download the CPT cTrader desktop version from our website at https://www.cptmarkets.com.',
  mac:'There is currently no MAC version, but we provide a CPT cTrader web version. Please click here.',
  clickto:'点击访问',
